...
module.exports = {
  ...
  getActivityUser: function (activityId, openid, callback) {
    db.collection('activity_apply').where({
      activity: {
        _id: activityId
      },
      _openid: openid
  ...
  //参加活动
  addActivityUser: function(activity_join, callback) {
    db.collection('activity_join').add({
      data: activity_join
    }).then(res => {
  ...
  findPageActivityUser: function(activityId, start, count, callback) {
    // 查询参与活动的人员
    var dbQuery = db.collection('activity_join').where({
      activity: {
        _id: activityId
      },
    });
