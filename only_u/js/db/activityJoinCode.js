...
module.exports = {
  ...
  getActivityCode: function(activityId, callback) {
    ...
    //查询所有活动参与码，若超过5分钟，则删除，有5分钟之内的则返回
    db.collection('activity_join_code').where({
  ...
  addActivityCode: function(activityId, code, callback) {
    ...
    db.collection('activity_join_code').add({
      data: activityCode
    }).then(res => {
  ...
  //根据id删除活动参与码
  removeActivityCodeById: function(codeId) {
    db.collection('activity_join_code').doc(codeId).remove().then(res => {
